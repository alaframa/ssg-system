...
export async function POST(req: NextRequest) {
  ...
  if (
    ["AGEN", "INDUSTRI"].includes(body.customerType) &&
    !body.address?.trim()
  ) {
  ...
  if (body.phone?.trim()) {
    ...
    if (!/^(\+62|62|0)[0-9]{8,13}$/.test(phoneClean)) {
      errors.phone =
        "Phone must be Indonesian format, e.g. 0812xxxxxxx or +62812xxxxxxx.";
    }
  }
  ...
  if (
    body.creditLimitKg12 !== undefined &&
    (isNaN(Number(body.creditLimitKg12)) || Number(body.creditLimitKg12) < 0)
  ) {
    errors.creditLimitKg12 = "Must be a non-negative integer.";
  }
  if (
    body.creditLimitKg50 !== undefined &&
    (isNaN(Number(body.creditLimitKg50)) || Number(body.creditLimitKg50) < 0)
  ) {
    errors.creditLimitKg50 = "Must be a non-negative integer.";
  }
  ...
  if (existing)
    return NextResponse.json(
      { errors: { code: `Customer code "${code}" already exists.` } },
      { status: 422 },
    );
  ...
  try {
    const customer = await prisma.customer.create({
      data: {
        ...
        creditLimitKg12: Number(body.creditLimitKg12) || 0,
        creditLimitKg50: Number(body.creditLimitKg50) || 0,
